...
public class NumSpan extends ReplacementSpan {
    final static String TAG = NumSpan.class.getSimpleName();

    private Context mContext;
    private int mBgColorResId = Color.parseColor("#FF4C6D99");; //Icon背景颜色
    ...
    public NumSpan(Context context, int bgColorResId) {
        //初始化默认数值
        initDefaultValue(context, bgColorResId);
        //计算背景的宽度
        this.mBgWidth = caculateBgWidth("0");
        //初始化画笔
        initPaint();
    }
    ...
    private float caculateBgWidth(String text) {
        if (text.length() > 0) {
            ...
            float padding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, mContext.getResources().getDisplayMetrics());
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        canvas.drawText(text, start, end, x + mBgWidth / 2, bgStartY + (mBgHeight - textRectHeight) / 2 - fontMetrics.top, textPaint);
    }
}
