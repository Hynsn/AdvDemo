...
Java_com_example_opensl_AudioPlayer_play_1assets(JNIEnv *env, jobject instance, jobject assetManager, jstring filename) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_opensl_AudioPlayer_play_1uri(JNIEnv *env, jobject instance, jstring uri) {
    SLresult result;
    release();
...
size_t getPcmData(FILE* file,void **pcm) {
    while(!feof(file)){
        size_t len = fread(buff, 1, 44100 * 2 * 2, pcmFile);
        LogI("%s : %d", "read ",len);
        if(len == 0) {
            LogI("%s", "play end");
        }
            ...
            LogI("%s tid: %d pid: %d", "playing",tid,pid);
        }
        *pcm = buff;
...
void pcmBufferCallBack(SLAndroidSimpleBufferQueueItf bf, void * context) {
    LogI("%s", "buffer Callback");
    //assert(NULL == context);

    size_t ret = getPcmData(pcmFile,&buffer);
    if(buffer && ret!=-1){
        (*pcmBufferQueue)->Enqueue(pcmBufferQueue, buffer, ret);
    }
    else{
        if(pcmFile!=NULL){
            fclose(pcmFile);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_opensl_AudioPlayer_play_1pcm(JNIEnv *env, jobject instance,
                                                                 jstring pamPath_) {
    release();
    ...
    result = (*engineEngine)->CreateAudioPlayer(engineEngine, &pcmPlayerObject, &dataSource, &dataSink, 3, ids, req);
    //初始化播放器
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_opensl_AudioPlayer_sendPcmData(JNIEnv *env, jobject instance,
    ...
    LogI("size is %d queue size is %d", size, wlQueue->getPcmdataSize());
